...
export default function({ store, Vue, router }) {
    ...
    axios.interceptors.response.use(
        ...
        err => {
            if (
                ...
                err.response.data.message === "Token Expired/Invalid"
            ) {
                if (!isRefreshing) {
                    isRefreshing = true;
                ...
                ["Bearer token invalid", "Refresh token not found"].includes(
    ...
    Vue.prototype.$get = function(path, queryParams) {
        return new Promise((resolve, reject) => {
            this.$http
                .get(this.$api(path), { params: queryParams })
                .then(res => {
                    resolve(res);
                })
                .catch(err => {
