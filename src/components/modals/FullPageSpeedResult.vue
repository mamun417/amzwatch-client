<template>
    <q-dialog :value="active" @hide="$emit('update:active', false)">
        <q-card>
            <q-card-section>
                <div
                    class="text-center text-primary text-h6 text-weight-bold q-mb-sm"
                >
                    Full Results
                </div>
                ...
                >
                    <q-item
                        v-if="
                            fullSpeedResultPage.meta.lhr_desktop_result.audits[
                                key
                            ].displayValue
                        "
                    ...
                    >
                        <q-item-section class="text-weight-bold" no-wrap>
                            <template v-if="$q.screen.lt.sm">
                                <q-item-label class="text-center q-my-sm">{{
                                    $_.startCase(
                                        $_.replace(
                                            key,
                                            new RegExp("-", "g"),
                                ...
                                <q-item-label>
                                    ...
                                    >
                                        <q-icon
                                            size="30px"
                                            :name="
                                                device === 'lhr_desktop_result'
                                                    ? 'desktop_mac'
                                                    : 'phone_android'
                                            "
                                            :color="
                                                $parent.calculateScoreType(
                                                    fullSpeedResultPage.meta[
                                                        device
                                                    ].audits[key].score * 100
                                                ).color
                                            "
                                        />
                                        <div
                                            v-if="
                                                fullSpeedResultPage.meta.hasOwnProperty(
                                                    device
                                                ) &&
                                                    fullSpeedResultPage.meta[
                                                        device
                                        ...
                                        >
                                            {{
                                                fullSpeedResultPage.meta[device]
                        ...
                        <q-item-section v-if="!$q.screen.lt.sm" no-wrap side>
                            ...
                            >
                                <span
                                    ...
                                    >{{
                                        fullSpeedResultPage.meta[device].audits[
                                            key
                                        ].displayValue
                                    }}</span
                                >
